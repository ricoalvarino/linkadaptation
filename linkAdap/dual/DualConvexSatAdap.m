classdef DualConvexSatAdap < DualLinkAdap
    %DualConvexSatAdap Algorithm that performs adaptation in the RL of LMS
    %channel using information of both RL (delayed) and FL (possibly
    %uncorrelated)
    %Adaptation is performed via LUT: a_0 + a_1*SNR_f + (1-a_1)*SNR_ol 
    %
    ...
    methods
        function o = DualConvexSatAdap(mcsSet,targetPER)
        ...
        function addFeedback(this,fb)
            ...
            %Update coefficients
            error = 1-fb.success;
            norm = (10^2+(fbSNRol-fbSNRcl)^2);
            this.a0 = this.a0 - 10*this.mu/norm*(error - this.p0);
            this.a1 = this.a1 - this.mu/norm*(error-this.p0)*(fbSNRcl-fbSNRol);
        ...
        function mcs = getMCS(this,param)
            snrTot = this.a0 + this.a1*this.curSNRcl + (1-this.a1)*this.curSNRol;
            mcs = 1;
